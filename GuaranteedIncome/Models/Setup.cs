    ...
    public class Setup
    {
        ...
        public Data ReturnData()
        {
            ...
            if (isDeferred == true)
            {
                ...
                data.Fixed = df.CalculateReturns(age, retireAge, deathAge, .05, .03, amount, TaxType, status, income);
                data.Variable = dv.CalculateReturns(age, retireAge, deathAge, .05, .03, amount, TaxType, status, income);
                data.FixedIndexed = dfi.CalculateReturns(age, retireAge, deathAge, .05, .03, amount, TaxType, status, income);
                data.Brokerage = b.CalculateReturns(age, retireAge, deathAge, .05, .03, amount, TaxType, status, income);
            }
            else
            {
